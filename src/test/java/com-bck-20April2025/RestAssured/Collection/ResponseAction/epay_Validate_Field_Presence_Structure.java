...
import com.RestAssured.Collection.pojo.epay_DirectRequest_Root;
import com.RestAssured.Collection.response.epay_ResponseData;


public class epay_Validate_Field_Presence_Structure{

	//public void 
	epay_ResponseData responseData;
	public epay_Validate_Field_Presence_Structure(epay_ResponseData responseData) {
	...
	{
		System.out.println("\n Reponse code : "+responseData.getResponseCode());
		if(responseData.getResponseCode().equals("00"))
		{
		...
		{
			Assert.assertNull(responseData.getApprovalRefNo(),"Verify ApprovalRefNo. is not Null");
		}
	}
	public void Validate_ResponseCode()
	{
		switch(responseData.getResponseCode())
		{
			...
			case "VALIDATION_ERR":
			{
				Assert.assertNotNull(responseData.getResponseMessage(),"Verify Response Message");
				Assert.assertNotNull(responseData.getResponseDescription(), "Verify Response Description");
				break;
			}
		}
	}
		...
		for(int i=0;i<responseData.getAdditionalInformation().size();i++)
		{
			...
			if(responseData.getResponseCode()=="00")
			{
				...
				if(i==8)
				{
					...
					Assert.assertNotNull(responseData.getAdditionalInformation().get(i).getValue(),"Verification of Additional Information Value at Position : "+i);	
				//	Assert.assertEquals(responseData.getAdditionalInformation().get(i).getName(), "BillDate","Verification of Additional Information Name at Position : "+i);
					String value  =responseData.getAdditionalInformation().get(i).getValue();
					    
				}
			}
			else {
				Assert.assertNull(responseData.getAdditionalInformation().get(i).getValue(),"Verification of Additional Information Value at Position : "+i);	
				//
			}
	...
	public void Validate_OperatorRefId()
	{
		...
		{
			Assert.assertNull(responseData.getOperatorRefId(),"Verify Operator Ref Id is not null");
		}
	}
